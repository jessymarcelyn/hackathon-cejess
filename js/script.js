...
function showResult(){
    ...
    if (userScore == 10) {
        ...
        const url = 'update_quiz.php';

        // Prepare the data to send to PHP
        const data = new FormData();
        data.append('userId', userId);  // Passing the userId
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    console.log('Score updated successfully:', xhr.responseText);
                } else {
                    console.error('Error updating score:', xhr.status);
                }
            }
        };
        xhr.send(data);
        ...
        let scoreTag = '<span>Anda mendapat <p>'+ userScore +'</p> dari <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
}
...
function queCounter(index){
    ...
    let totalQueCounTag = '<span><p>'+ index +'</p> of <p>'+ questions.length +'</p> Pertanyaan</span>';
